// Días hábiles manuales para cada mes del año 2025
// Formato: YYYY-MM-DD
// Solo incluye lunes a viernes (1-5)

export const diasHabiles2025: Record<number, string[]> = {
  1: [ // Enero 2025
...
// Función para obtener días hábiles de un mes específico
export function getDiasHabilesManuales(mes: number, anio: number): string[] {
  // Por ahora solo tenemos 2025, pero podemos expandir esto
  if (anio === 2025 && mes >= 1 && mes <= 12) {
    return diasHabiles2025[mes];
  }
  
  // Si no tenemos datos para ese año/mes, devolver array vacío
  console.warn(`No hay datos manuales para ${mes}/${anio}`);
  return [];
}

// Función para verificar que los días son correctos
export function verificarDiasHabiles(mes: number, anio: number): void {
  const dias = getDiasHabilesManuales(mes, anio);
  console.log(`=== VERIFICACIÓN DÍAS HÁBILES ${mes}/${anio} ===`);
  console.log(`Total días hábiles: ${dias.length}`);
  console.log('Primer día:', dias[0]);
  console.log('Último día:', dias[dias.length - 1]);
  
  // Verificar que todos son lunes a viernes
  const diasIncorrectos = dias.filter(fecha => {
    const diaSemana = new Date(fecha).getDay();
    return diaSemana === 0 || diaSemana === 6; // Domingo o Sábado
  });
  
  if (diasIncorrectos.length > 0) {
    console.error('DÍAS INCORRECTOS (sábados/domingos):', diasIncorrectos);
  } else {
    console.log('✅ Todos los días son hábiles (lunes a viernes)');
  }
  
  // Verificar que están en el mes correcto
  const diasFueraDeMes = dias.filter(fecha => {
    const fechaObj = new Date(fecha);
    return fechaObj.getMonth() + 1 !== mes || fechaObj.getFullYear() !== anio;
  });
  
  if (diasFueraDeMes.length > 0) {
    console.error('DÍAS FUERA DEL MES:', diasFueraDeMes);
  } else {
    console.log('✅ Todos los días están en el mes correcto');
  }
